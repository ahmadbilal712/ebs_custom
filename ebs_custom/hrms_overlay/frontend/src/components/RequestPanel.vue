...
import { myAdvances, teamAdvances } from "@/data/advances"
...
const myRequests = computed(() =>
	updateRequestDetails(myLeaves, myClaims, myShiftRequests, myAttendanceRequests, myAdvances)
)

const teamRequests = computed(() =>
	updateRequestDetails(teamLeaves, teamClaims, teamShiftRequests, teamAttendanceRequests, teamAdvances)
)

function updateRequestDetails(leaves, claims, shiftRequests, attendanceRequests, advances) {
	const requests = [leaves, claims, shiftRequests, attendanceRequests, advances].reduce(
	...
	const componentMap = {
		...
		"Employee Advance": EmployeeAdvanceItem,
	}
...
onMounted(() => {
	...
	useListUpdate(socket, "Employee Advance", () => teamAdvances.reload())
})
</script>
